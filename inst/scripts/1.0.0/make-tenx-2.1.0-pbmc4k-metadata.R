...
write.csv(file="../../extdata/1.0.0/metadata-tenx-2.1.0-pbmc4k.csv",
    data.frame(
        ...
        Description = sprintf("%s for 10X Genomics PBMC 4K single-cell RNA-seq dataset (version 2 chemistry, CellRanger 2.1.0)", c(
            "Raw count matrix", 
            "Filtered count matrix", 
            "Raw HDF5 matrix", 
            "Molecule information file")),
        RDataPath = file.path("DropletTestFiles", "tenx-2.1.0-pbmc4k", "1.0.0", files),
        BiocVersion="3.12",
        Genome="hg38",
