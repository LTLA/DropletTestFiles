...
#name tweak for consistency
files <- dir(file.path(target, "hiseq_2500_run2"), full.names = TRUE)
file.rename(
    files, 
    file.path(dirname(files), paste0("molecule_info_", basename(files)))
)

#metadata
files <- dir(target, recursive = TRUE, full.names = TRUE)
sequencer <- ifelse(grepl("4000", files), "HiSeq 4000", "HiSeq 2500")
experiment <- ifelse(grepl("run2", files), 2, 1)
barcode <- regmatches(files, regexpr("[A-Z][1-9]", files))
title <- paste0(sequencer, "-sequenced scRNAseq mammary epithelial cells (expt. ", experiment, " barcode ", barcode, ")")
desc <- paste0("Molecule info file for ", sequencer, "-sequenced scRNAseq molecule info (experiment ", experiment, ", using 10x barcode ", barcode, "; 10x 3' v2 kit)")
meta <- data.frame(
    Title = title,
    Description = desc,
    RDataPath = files,
